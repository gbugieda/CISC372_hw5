...
//Returns: Nothing
void* convolute(void* s){
    threadArgs *t;
    t = (threadArgs *)s;
    Image* srcImage = t->src;
    Image* destImage = t->dest;
    long int rank = t->rank;
    int interval = srcImage->height / numThreads;
    ...
    int end;
    if (start == (numThreads-1) * interval){
        end = srcImage->height;
    } 
    else{
        end = ((rank + 1) * interval);
...
int main(int argc,char** argv){
    ...
    threadArgs argArray[numThreads];
    for (long int i = 0; i < numThreads; i++){
        argArray[i].src = &srcImage;
        argArray[i].dest = &destImage;
        argArray[i].rank = i;
    }
    for (long int i = 0; i < numThreads; i++){        
        pthread_create(&threads[i], NULL, convolute,(void*)&argArray[i]);
    }
